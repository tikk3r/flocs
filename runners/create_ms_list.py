...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate an input file for LINC containing measurement sets and, optionally, the calibrator solutions."
    )
    parser.add_argument(
        "mspath", type=str, help="Path where input measurement sets are located."
    )

    vlbiparser = parser.add_argument_group("== lofar-vlbi specific settings ==")
    vlbiparser.add_argument(
        "--vlbi",
        action="store_true",
        default=False,
        help="Triggers alternate mode to generate a config file for the lofar-vlbi pipeline.",
    )
    vlbiparser.add_argument(
        "--solset",
        type=cwl_file,
        default="",
        help="Path to the final LINC target solution file (usually cal_solutions.h5).",
    )
    vlbiparser.add_argument(
        "--ddf_solsdir",
        type=cwl_dir,
    ...
    vlbiparser.add_argument(
        ...
        type=cwl_file,
        default="null",
        help="The solution tables generated by the DDF pipeline in an HDF5 format.",
    )
    vlbiparser.add_argument(
        "--configfile",
        type=cwl_file,
    ...
    vlbiparser.add_argument(
        ...
        type=str,
        default="TGSSphase",
        help="Name of the soltab with LINC target phase solutions.",
    )
    vlbiparser.add_argument(
        "--reference_stationSB",
        type=int,
    ...
    vlbiparser.add_argument(
        ...
        type=bool,
        default=False,
        help="[Split-Directions] self calibrate the split out targets.",
    )
    vlbiparser.add_argument(
        "--linc", type=cwl_dir, default="", help="Path to LINC installation directory."
    )
    ...
    dparser.add_argument(
        ...
        type=int,
        default=15,
        help="Size of the window over which a value is interpolated. Should be odd.",
    )
    dparser.add_argument(
        "--ampRange",
        type=float,
    ...
    dparser.add_argument(
        ...
        type=bool,
        default=True,
        help="Update WEIGHT_SPECTRUM column in a way consistent with the weights being inverse proportional to the autocorrelations (default: True).",
    )
    dparser.add_argument(
        "--apply_tec",
        type=bool,
        default=False,
        help="Apply TEC solutions from the calibrator (default: False).",
    )
    dparser.add_argument(
        "--apply_clock",
        type=bool,
        default=True,
        help="Apply clock solutions from the calibrator (default: True).",
    )
    dparser.add_argument(
        "--apply_phase",
        type=bool,
        default=False,
        help="Apply full phase solutions from the calibrator (default: False).",
    )
    dparser.add_argument(
        "--apply_RM",
        type=bool,
        default=True,
        help="Apply ionospheric Rotation Measure from RMextract (default: True).",
    )
    dparser.add_argument(
        "--apply_beam",
        type=bool,
        default=True,
        help="Apply element beam corrections (default: True).",
    )
    dparser.add_argument(
        "--gsmcal_step",
        type=str,
        default="phase",
        help="Type of calibration to be performed in the self-calibration step (default: phase)",
    )
    dparser.add_argument(
        "--use_target",
        type=bool,
        default=True,
        help="Enable downloading of a target skymodel (default: True).",
    )
    dparser.add_argument(
        "--selfcal",
        type=bool,
        default=False,
        help="Perform extensive self-calibration according to the LiLF scheme (recommended for LBA observations) (default: false)",
    )
    dparser.add_argument(
        "--selfcal_region",
        type=cwl_file,
    ...
    dparser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Minimal fraction of unflagged data to be accepted for further processing of the data chunk (default: 0.5)",
    )
    dparser.add_argument(
        "--num_SBs_per_group",
        type=int,
    ...
    demixparser.add_argument(
        ...
        type=str,
        default=None,
        help="If true force demixing using all sources of demix_sources, if false do not demix (default: null, automatically determines sources to be demixed according to min_separation).",
    )
    demixparser.add_argument(
        "--lbfgs_historysize",
        type=int,
    ...
    skyparser.add_argument(
        ...
        type=str,
        default="TGSS",
        help="Choose the target skymodel from TGSS ADR (TGSS) or the new Global Sky Model (GSM) (default: TGSS).",
    )
    skyparser.add_argument(
        "--skymodel_fluxlimit",
        type=float,
    ...
    if args["vlbi"]:
        ...
        if (args["delay_solset"]["path"].endswith("h5")) or (
            args["delay_solset"]["path"].endswith("h5parm")
        ):
