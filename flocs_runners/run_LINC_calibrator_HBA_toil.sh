...
if [[ -z "$SIMG" ]]; then
    ...
    if [[ "$CONTAINERSTR" == *"apptainer"* ]]; then
        ...
        export APPTAINERENV_PATTH=$LINC_DATA_ROOT/scripts:\$PATH
        export APPTAINERENV_PYTHONPATH=$LINC_DATA_ROOT/scripts:\$PYTHONPATH
    else
        export SINGULARITY_CACHEDIR=$RUNDIR/simgcache
